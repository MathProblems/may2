...
def infer(q,a,VERBOSE):
    ...
    for k in range(len(wps)):
        ...
        for i in eqidxs:
            ...
            mix.append((op_val*scores[i],i,guess))
        ...
        srt3 = sorted(mix,reverse=True)
        lg = srt2[0]
        gg = srt[0]
        mg = srt3[0]
        score, i, guess = lg
        if guess == answ or guess/100 == answ or answ/100 == guess:
            localthresh.append((score,i,1))
        else:
            localthresh.append((score,i,0))

        score, i, guess = gg
        if guess == answ or guess/100 == answ or answ/100 == guess:
            globalthresh.append((score,i,1))
        else:
            globalthresh.append((score,i,0))

        score, i, guess = mg
        if guess == answ or guess/100 == answ or answ/100 == guess:
            mixthresh.append((score,i,1))
        else:
            mixthresh.append((score,i,0))
        ...
        continue
    print(right,guesses)
    outfile = sys.argv[1].split(".")[0][-1]
    pickle.dump((localthresh,globalthresh,mixthresh),open("data/thresh"+outfile+".pickle",'wb'))
