...
def entityextract(story):
    ...
    for j,s in enumerate(story):
        ...
        surfaces = [x.lower() for x in surfaces]
        thesesetssofar = [x for x in sets if x[0]//1000 == j]
        if 'each' in surfaces:
            ...
            ranges = []
            for x in [',','but','and']:
                tmp = [n for n,y in enumerate(surfaces) if y==x]
                ranges.extend(tmp)
            low = 0
            high = len(surfaces)
            if ranges:
                print(ranges)
                lows = [x for x in ranges if x < eachi]
                if lows:
                    low = lows[-1]
                highs = [x for x in ranges if x > eachi]
                if highs: high = highs[0]
            targets = [x for x in thesesetssofar if int(x[1].widx) > low and int(x[1].widx) < high and x[1].num.isdigit()]
            if targets:
                target = targets[-1][1]
                sets.append(((j*1000)+eachi,aset('each',target.entity,'',j*1000+eachi)))
                if "$" in surfaces[eachi-3:eachi]:
                    sets[-1][1].contains = "dollar"
            else:
