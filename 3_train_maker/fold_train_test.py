...
traine = [x for i,x in enumerate(e) if i in train]
...
with open('data_for_roy/train'+sys.argv[1],'w') as f:
    for i in range(len(trainq)):
        f.write(trainq[i])
        f.write(traine[i])
        f.write(traina[i])
with open('data_for_roy/test'+str(sys.argv[1]),'w') as f:
