...
def eqvector(a,b,problem,story,target,feats=False):
    ...
    for j in range(len(story)):
        for i,x in enumerate(story['sentences'][j]['words']):
            allwords.append((j*1000+i,x[0]))
    middle = [x[1] for x in allwords if x[0]>a.idx and b.idx>x[0]]
    left = [x[1] for x in allwords if x[0]<a.idx]
    right = [x[1] for x in allwords if x[0]>b.idx]
    #left = [x[1] for x in allwords if x[0]//1000==sp[0].idx//1000]
    #right = [x[1] for x in allwords if x[0]//1000==sp[1].idx//1000]
    keywords = ["times","total","together","more","less","add","divide","split","equal","equally",'start','first','left','altogether','today','yesterday','now','then','after','beginning','end','begin','each','only']
    for k in keywords:
                    ...
                    for bw in bvl:
                        ...
                        for bsyn in bsyns:
                            if asyn._pos == bsyn._pos: 
                                try:
                                    sim = 1/(1+bsyn.lin_similarity(asyn,brown_ic))
                                except:
                                    sim = 2
                                if sim < dist:
                                    dist = sim
...
def combine(a,b,op):
    ...
    for k in a.__dict__:
        if k == "num":
            ...
            '''
            if b.__dict__[k] == None:
                c.__dict__[k] = a.__dict__[k]
            else:
                c.__dict__[k]= b.__dict__[k]
            '''
    #print(c.__dict__)
...
def entityextract(story):
    ...
    for j,s in enumerate(story):
        ...
        nums.extend([(x[2],x[1]) for x in deps if x[0]=="prep_of" and (floatcheck(x[1].split("-")[0]) or x[1].rsplit("-",maxsplit=1)[0] in ['half','third','quarter'])])
        for w,n in nums:
            n,nidx = n.split("-")
            nidx = int(nidx)-1
...
def containers(sets,story):
    for j,s in enumerate(story):
        ...
        for e in thissentsets:
            ...
            adjs = [x for x in deps if x[1]==esurface and x[0] in ['amod']]
            ofs = [x for x in deps if x[1]==esurface and x[0] in ['prep_of']]
            if ofs:
                e[1].unit = e[1].entity
                oidx = int(ofs[0][2].split("-")[1])-1
                lem = s['words'][oidx][1]["Lemma"]
                e[1].entity = lem
            ...
            if elocs:
                ...
                adjsurf = [x[2] for x in adjs]
                elocs = [x[2].split("-")[0] for x in deps if x[0] in ['prep_in','prep_on','prep_at'] and x[1] in adjsurf]
                if elocs:
                    e[1].location = ' '.join(elocs)
                else:
                    vbsurf = [x[1] for x in vbs]
                    elocs = [x[2].split("-")[0] for x in deps if x[0] in ['prep_in','prep_on','prep_at'] and x[1] in vbsurf]
                    if elocs:
                        e[1].location = ' '.join(elocs)
