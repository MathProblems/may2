...
import jAll_train_local #import get_k_eqs
...
def make_eq(q,a,equations):
    ...
    for k in range(len(wps)):
        if k%3!=2:continue
        answers = jAll_train_local.get_k_eqs(equations[k],g=True,a=True)
        answers = [x for x in answers if x[1].split()[-2]=='=']
        answers = [x for x in answers if x[1].split()[-1]=='x']
        print(answers);print(k)
        if answers == []:wrong +=1; continue
        seeneq = []
        seen = []
        for x in answers:
...
if __name__=="__main__":
    ...
    jAll_train_local.LEN = len(q)
    q = q + q2
    jAll_train_local.LEN2 = len(q)
    LEN2 = len(q)
    a = a + a2 + a3
    q = q+q3
    e = [i for i in range(len(q))]

    print(jAll_train_local.LEN, jAll_train_local.LEN2)
