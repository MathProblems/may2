...
def make_eq(q,a,equations):
    ...
    for k in range(len(wps)):
        ...
        if answers == []: continue
        simpls = [x for x in answers if "x =" in x or "= x" in x]
        if simpls: answers = simpls
        else:
            simpls = [x for x in answers if x.split()[-2]=='=']
            if not simpls:
                continue
...
#eqsdir = "data.javad.out.2"
#eqsdir = "data.javad.out"
