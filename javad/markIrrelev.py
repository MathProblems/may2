...
def oneEnt(sets):
    ...
    if len(notx)==1:     
        ...
        if not x:
            sets.append((0,aset('x',list(notx)[0],'none',0)))
        else:
            x = x[0]
...
def irr(sets):
    print([(i,(x[1].num,x[1].entity)) for i,x in enumerate(sets)])
    rem = input()
    if rem.isdigit():
        sets.pop(int(rem))
        print('deleted ', rem)
...
def makesets(story,irrelev=False):
...
if __name__ == "__main__":
    ...
    for k in range(len(q)):
        ...
        irrelevSets.append((k,sets))
        #for s in sets: s[1].details()
    pickle.dump(irrelevSets,open('irrelevSets.pickle','wb'))
