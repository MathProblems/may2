...
def training(a,b,problem,story,target,j,order,sp):
    #this function take the trips and creates positive and negative training instances from them
    
    if j == 0:
        j=-1
    vec = [j,order]
    vec.extend(makesets.eqvector(a,b,problem,story,target,sp))
...
def make_eq(q,a,equations):
    ...
    for k in range(len(wps)):
        ...
        for j,eq in answers:
            ...
            consts = consts.split(" = ")
            sp = (objs[consts[0].split(" ")[-1]][1],objs[consts[1].split(" ")[0]][1])
            ...
            tdata.append(training(sides[0],sides[1],problem,story,target,j,order,sp))
