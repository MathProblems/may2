...
def infer(q,a,cutoff,VERBOSE):
    ...
    for k in range(len(wps)):
        ...
        for i in eqidxs:
            ...
            vec.extend(vectorize_eqn.vec(scores[i],guess,ogeq.strip().split(" "),integerproblem,equalsmatch[i],contmatch[i],problem,failurerate[i],fivescores[i]))
            print(eq,vec)
            '''
            vec = []
...
if __name__=="__main__":
    ...
    for i in range(len(q)):
        g,b = pickle.load(open('eqs/'+str(i)+'.eqs','rb'))
        eqs.append(a+b)
    '''
