...
def eqvector(a,b,problem,story,target,feats=False):
    ...
    for li in ["times","total","together","more","less","add","divide","split","left","equal","equally","now",'left','start','altogether','today','yesterday','now','then','after','beginning','end','begin','each','only']:
        if li in problem:
            vec.append(1)
        else:
            vec.append(0)
...
def vector(a,b,problem,story,target,feats=False):
    ...
    for item in [',','and','but',"times","total","together","more","less","add","divide","split","equal","equally"]:
    ...
    for li in ["times","total","together","more","less","add","divide","split","equal","equally",'and',',','but']:
        if li in asent:
            vec.append(1)
        else:
            vec.append(0)
    ...
    vec.append(int( a.origs == b.origs))
...
def oneEnt(sets):
    notx = set([x[1].entity for x in sets if x[1].num!='x'])
    if len(notx)==1:
        x = [x[1] for x in sets if x[1].num=='x'][0]
...
def oneSet(sets,story):
    ...
    if len(qsets)==1:
        ...
        if 'week' in allwords or "Week" in allwords:
            sets.append((0,aset('7','days','week',0)))
