...
def training(a,b,problem,story,target,j,order,score):
...
def make_eq(q,a,equations):
    ...
    for k in range(len(wps)):
        ...
        good = [x for x in answers if x[0]==1]
        bad = [x for x in answers if x[0]==0][:len(good)]
        answers = good+bad
        if answers == []: continue
        ...
        for j,eq in answers:
            ...
            tdata.append(training(sides[0],sides[1],problem,story,target,j,order,score))
